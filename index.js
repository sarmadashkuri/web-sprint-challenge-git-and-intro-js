...
console.log('task 3', artists[8].name);
...
console.log('task 4', getArtistByIndex[artists, 0]);
...
function listOfNames(array) {
  ...
  for(let i = 0; i < copyArray.length; i++){
    newArray.push(copyArray[i].name);
  }
  return newArray;
}
...
function addArtist(array, p2, p3, p4, p5, p6) {
  const copyArray = [...array];
  const addedArtist = {};
  addedArtist.name = p2;
  addedArtist.years = p3;
  addedArtist.genre = p4;
  addedArtist.nationality = p5;
  addedArtist.bio = p6;
  copyArray.push(addedArtist);
  return copyArray;
}
...
function lotsOfArt(array) {
  const hundredArtists = [];
  for(let i = 0; i < array.length; i++){
    if(array[i].paintings > 100){
      hundredArtists.push(array[i].name);
    }
  }
